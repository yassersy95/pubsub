...
def sub(project_id: str, subscription_id: str, timeout: float = None) -> None:
    ...
    def callback(message: pubsub_v1.subscriber.message.Message) -> None:
        ...
        j = json.loads(str(message.data.decode('ascii').replace("'", '"')))["data"]
        sql = 'insert into weather_trans(deviceid, temperature, latitude, longitude, time, isanomal) values(%s, %s, %s, %s, %s, %s)'
        print(sql)
        conn = pymysql.connect(host='127.0.0.1', port=3306, user='weather', passwd='weather', db='weather')
        cur = conn.cursor()
        a = cur.execute(sql, (
            j["deviceId"], j["temperature"], j["location"]["latitude"], j["location"]["longitude"], j["time"], -1))
        conn.commit()
        print('suc')
        # for r in cur:
        #    print(r)
